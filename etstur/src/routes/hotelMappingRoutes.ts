  ...
  try {
    ...
    if (search) {
      where.OR = [
        ...
        { etsturHotelId: { contains: String(search) } },
      ];
    }
...
router.post("/", async (req: Request, res: Response) => {
  try {
    const { internalHotelId, etsturHotelId, destinationUrl, hotelName, isActive } =
      req.body as HotelMappingInput;

    if (!internalHotelId || !etsturHotelId || !destinationUrl) {
      res.status(400).json({
        error: "internalHotelId, etsturHotelId ve destinationUrl zorunlu.",
      });
      return;
    }
  ...
  try {
    ...
    const mapping = await prisma.hotelMapping.update({
      ...
      data: {
        ...
        ...(etsturHotelId !== undefined && { etsturHotelId }),
